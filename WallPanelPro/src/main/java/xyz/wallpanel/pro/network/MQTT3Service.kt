...
class MQTT3Service(
    ...
    override val isReady: Boolean
        get() = mReady.get() && mqtt3AsyncClient?.state?.isConnected == true
    ...
    override fun close() {
        mqtt3AsyncClient?.let { client ->
            ...
            val offlineSent = connectedBaseTopic?.takeIf { client.state.isConnected }?.let { topic ->
                val offlineMessage =
                    Mqtt3Publish.builder().topic("$topic${CONNECTION}")
    ...
    private fun disconnectQuietly(client: Mqtt3AsyncClient) {
        try {
            client.disconnect()
        } catch (e: Exception) {
    ...
    private fun initializeMqttClient() {
        try {
            mqttOptions?.let { mqttOptions ->
                ...
                clientConnect.cleanSession(mqttOptions.connectionTest)
                if (!mqttOptions.connectionTest) {
